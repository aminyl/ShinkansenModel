...
public class SeatGenerator : MonoBehaviour
{
	...
	void GenSeats (Vector3 potision){
		GameObject _seat = Instantiate (seat);
		_seat.transform.position = potision;
		_seat.transform.parent = transform;
	}
	...
	void GenerateSeats ()
	{
		for (int i = 0; i < COL_LEFT_N; i++) {
			for (int j = 0; j < ROW_N; j++) {
				GenSeats(new Vector3 (i * WIDTH, 0, -j * PITCH));
			}
		}
		float _offset = CENTER_MERGIN + WIDTH * COL_LEFT_N;
		for (int i = 0; i < COL_RIGHT_N; i++) {
			for (int j = 0; j < ROW_N; j++) {
				GenSeats(new Vector3 (i * WIDTH + _offset, 0, -j * PITCH));
			}
		}
	}
}